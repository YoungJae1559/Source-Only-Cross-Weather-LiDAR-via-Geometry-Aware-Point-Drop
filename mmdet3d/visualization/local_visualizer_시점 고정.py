...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def _dump_cam(self, path='ego_front_cam.json'):
        import json, open3d as o3d, numpy as np
        vc = self.view_control
        params = vc.convert_to_pinhole_camera_parameters()
        # 1) 핀홀 파라미터 저장(이것만으로 시점 완전 복원 가능)
        o3d.io.write_pinhole_camera_parameters(path, params)

        # 2) 사람이 보기 쉬운 정보(eye)만 덤프: get_front 없이 extrinsic으로 계산
        E = np.asarray(params.extrinsic)     # world -> cam
        R, t = E[:3, :3], E[:3, 3]
        eye = (-R.T @ t).tolist()            # 카메라 위치(월드 좌표)
        meta = {'eye_world': eye}
        with open(path.replace('.json', '_meta.json'), 'w') as f:
            json.dump(meta, f, indent=2)
        print(f'[CAM] saved -> {path}')
        print(f'[CAM] eye_world: {eye}')
        return False

    def _on_press_P(self, vis):
        try:
            self._dump_cam()   # ego_front_cam.json 생성
        except Exception as e:
            print('[CAM] dump failed:', e)
        return False            # False면 불필요한 리렌더링 없음
    ...
    def _initialize_o3d_vis(self) -> Visualizer:
        ...
        # 일부 Open3D 빌드는 key_action 콜백이 없음 → fallback 처리
        try:
            o3d_vis.register_key_action_callback(glfw_key_space, self.space_action_callback)
        except Exception:
            o3d_vis.register_key_callback(glfw_key_space, self.space_action_callback)
        o3d_vis.register_key_callback(glfw_key_right, self.right_callback)

        # P/p: 현재 카메라 파라미터 JSON 덤프
        o3d_vis.register_key_callback(glfw_key_P, self._on_press_P)
        o3d_vis.register_key_callback(glfw_key_p, self._on_press_P)
