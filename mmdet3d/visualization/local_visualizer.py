...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def set_points(self,
        ...
        if render_option is not None:
            ...
            render_option.background_color = np.asarray([0, 0, 0]) # background = black
            #render_option.background_color = np.asarray([1.0, 1.0, 1.0]) # background = white
    ...
    def show(self,
        ...
        if hasattr(self, 'o3d_vis'):
            # ──────────────────────────────────────────────────────────────────────────────
            # Non‑interactive branch: skip event loop and capture immediately
            # Enable by setting self.non_interactive = True from the caller.
            # ──────────────────────────────────────────────────────────────────────────────
            if getattr(self, 'non_interactive', False):
                ...
                # Apply fixed camera json if exists
                try:
                    import os, open3d as o3d
                    cam_json = os.path.abspath(getattr(self, 'fixed_cam_path', 'ego_front_cam.json'))
                    if os.path.exists(cam_json):
                        params = o3d.io.read_pinhole_camera_parameters(cam_json)
                        self.view_control.convert_from_pinhole_camera_parameters(params)
                except Exception as e:
                    print('[CAM] apply failed (non_interactive):', e)
                ...
                try:
                    ...
                    # persist current viewport for next frame
            ...
            # Apply fixed camera json before capture
            try:
                import os, open3d as o3d
                cam_json = os.path.abspath(getattr(self, 'fixed_cam_path', 'ego_front_cam.json'))
                if os.path.exists(cam_json):
