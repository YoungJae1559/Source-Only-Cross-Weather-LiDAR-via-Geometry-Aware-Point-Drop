...
class SimulatedMatterAccumulation(BaseTransform):
    def __init__(self, rho=0.3, h_range=(0.05, 0.3), gamma_range=(0.3, 1.0), prob=1.0, separately_output=False):
        self.rho = float(rho)
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        mask = torch.rand(N, device=device) < self.rho
        if mask.any():
            h = torch.empty(N, device=device).uniform_(self.h1, self.h2)
            g = torch.empty(N, device=device).uniform_(self.g1, self.g2)
            pts = points.tensor
            pts[mask, 2] = pts[mask, 2] + h[mask]
            pts[mask, -1] = pts[mask, -1] * g[mask]
            points.tensor = pts
        if self.separately_output:
            input_dict["points_aug"] = points
    ...
    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}(rho={self.rho}, h_range=({self.h1}, {self.h2}), "
                f"gamma_range=({self.g1}, {self.g2}), prob={self.prob})")

@TRANSFORMS.register_module()
class SimulatedFuzzyRecognition(BaseTransform):
    def __init__(self, mode="percentile", p=0.2, alpha=0.04, ignore_index=255, prob=1.0, separately_output=False):
        self.mode = mode
        self.p = float(p)
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if self.mode == "exp_decay":
            r = torch.sqrt(coord[:, 0] ** 2 + coord[:, 1] ** 2 + coord[:, 2] ** 2)
            Ie = I * torch.exp(-self.alpha * r)
            tau = torch.quantile(Ie, torch.tensor(self.p, device=Ie.device))
            mask = Ie < tau
        else:
            tau = torch.quantile(I, torch.tensor(self.p, device=I.device))
            mask = I < tau
        if isinstance(labels, np.ndarray):
            labels = labels.copy()
            labels[mask.cpu().numpy()] = self.ignore_index
            input_dict["pts_semantic_mask"] = labels
        else:
            lbl = labels.clone()
            lbl[mask] = self.ignore_index
            input_dict["pts_semantic_mask"] = lbl
        if self.separately_output:
    ...
    def __repr__(self) -> str:
        return (f"{self.__class__.__name__}(mode={self.mode}, p={self.p}, alpha={self.alpha}, "
                f"ignore_index={self.ignore_index}, prob={self.prob})")
