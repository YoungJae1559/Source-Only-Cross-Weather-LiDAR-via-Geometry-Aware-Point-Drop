...
#                  norm_intensity_sigmoid: bool = False,
#                  backend_args: Optional[dict] = None,
#                  # NEW: flexible loading controls
#                  auto_load_dim: bool = True,
#                  candidate_load_dims: Tuple[int, ...] = (5, 4, 6)) -> None:
#         self.shift_height = shift_height
#         self.use_color = use_color
#         if isinstance(use_dim, int):
#             use_dim = list(range(use_dim))
...
#         # Flexible options
#         self.auto_load_dim = auto_load_dim
#         self.candidate_load_dims = candidate_load_dims
...
#     def _infer_load_dim(self, raw: np.ndarray) -> int:
#         if raw.ndim == 2:
#             return int(raw.shape[-1])

#         # 1-D buffer: infer by divisibility
#         floats = int(raw.size)
#         valid = []
#         # Prefer explicitly configured load_dim when consistent
#         if self.load_dim is not None and self.load_dim > 0 and floats % self.load_dim == 0:
#             valid.append(self.load_dim)
#         # Then try candidates in order
#         for cand in self.candidate_load_dims:
#             if cand not in valid and floats % cand == 0:
#                 valid.append(cand)
#         if not valid:
#             raise RuntimeError(
#                 f"[LoadPointsFromFile] Cannot infer channel count: buffer floats={floats}, "
#                 f"tried load_dim={self.load_dim} and candidates={self.candidate_load_dims}. "
#                 f"File may be corrupted or not float32.")
#         # Pick the first valid option (preference order handled above)
#         chosen = valid[0]
#         # If multiple fit (e.g., divisible by 20 → 4 & 5), warn and choose preference
#         if len(valid) > 1:
#             warnings.warn(
#                 f"[LoadPointsFromFile] Multiple channel candidates fit {floats}: {valid}. "
#                 f"Choosing {chosen}. You can control order via candidate_load_dims or set load_dim explicitly.")
#         return chosen

#     def transform(self, results: dict) -> dict:
#         pts_file_path = results['lidar_points']['lidar_path']
#         raw = self._load_points(pts_file_path)

#         # Autodetect channel count if needed
#         chosen_dim = self._infer_load_dim(raw) if self.auto_load_dim else self.load_dim

#         # Final safety: ensure use_dim indices are valid for chosen_dim
#         if len(self.use_dim) > 0 and max(self.use_dim) >= chosen_dim:
#             raise AssertionError(
#                 f'Expect all used dimensions < chosen_dim ({chosen_dim}), got use_dim={self.use_dim} for file {pts_file_path}')

#         # Reshape (handles both 1-D and already 2-D arrays)
#         points = raw if raw.ndim == 2 else raw.reshape(-1, chosen_dim)
#         points = points[:, self.use_dim]
...
#             assert len(self.use_dim) >= 4, (
#                 f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}')
#             imin, imax = np.min(points[:, 3]), np.max(points[:, 3])
#             if imax > imin:
#                 points[:, 3] = (points[:, 3] - imin) / (imax - imin)
#         if self.norm_intensity_sigmoid:
#             assert len(self.use_dim) >= 4, (
#                 f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}')
#             points[:, 3] = sigmoid(points[:, 3])
...
#         if self.use_color:
#             assert len(self.use_dim) >= 6
#             attribute_dims = attribute_dims or {}
...
#     def __repr__(self) -> str:
#         repr_str = (self.__class__.__name__ + '('
#                     f'shift_height={self.shift_height}, '
#                     f'use_color={self.use_color}, '
#                     f'backend_args={self.backend_args}, '
#                     f'load_dim={self.load_dim}, '
#                     f'use_dim={self.use_dim}, '
#                     f'auto_load_dim={self.auto_load_dim}, '
#                     f'candidate_load_dims={self.candidate_load_dims}, '
#                     f'norm_intensity={self.norm_intensity}, '
#                     f'norm_elongation={self.norm_elongation})')
#         return repr_str
