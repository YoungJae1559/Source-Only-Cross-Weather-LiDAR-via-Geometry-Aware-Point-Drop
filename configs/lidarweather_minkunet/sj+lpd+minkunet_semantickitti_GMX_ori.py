...
# --- NEW: DDP wrapper to allow unused params safely ---
...
labels_map = {
    ...
    80: 17, 81: 18, 99: 19, 252: 0, 253: 6, 254: 5, 255: 7, 256: 4, 257: 4,
    258: 3, 259: 4
}
...
# Depth & Angle Selective Jittering
train_pipeline = [
...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        voxel_layer=dict(max_num_points=-1, point_cloud_range=[-100, -100, -20, 100, 100, 20], voxel_size=[0.05, 0.05, 0.05], max_voxels=(-1, -1))),
    backbone=dict(
        type='MinkUNetBackbone', in_channels=4, num_stages=4, base_channels=32,
        encoder_channels=[32, 64, 128, 256], encoder_blocks=[2, 2, 2, 2],
        decoder_channels=[256, 128, 96, 96], decoder_blocks=[2, 2, 2, 2], block_type='basic', sparseconv_backend='torchsparse'),
    decode_head=dict(
    ...
    train_cfg=dict(
        ...
        # --- NEW: Region‑DQN & GMX hyperparams ---
